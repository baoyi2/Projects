// This code is implemented on an Arduino UNO, with a Rich Shield TM1637
#define BUTTONK1 8	
#define BUTTONK2 9
#include <Wire.h>
#include "RichShieldTM1637.h"
#define CLK 10 //CLK of the TM 1637 IC connected to D10 of OPEN-SMART UNO R3
#define DIO 11 //DIO of the TM 1637 IC connected to D11 of OPEN-SMART UNO R3
TM1637 disp(CLK, DIO);
#define LED_GREEN 5
...
int calc()
{
    int x, y, count;
    count = z;
        if (digitalRead(BUTTONK2) == 0)  //if BUTTONK2 is pressed
        {
            if (digitalRead(BUTTONK1) == 1)//if BUTTONK1c is not pressed
            {
                z += 1; //add 1 to z
                delay(1000);
            }
        ...
        if (digitalRead(BUTTONK1) == 0)   //if BUTTONK2 is pressed
        {
            if (digitalRead(BUTTONK2) == 1) {//if BUTTONK1c is not pressed
                z -= 1; //minus 1 to z
                delay(1000);
            }
        }
...
seven segment display )and also display the array*/
{
    ...
    Disp[3] = count1 % 10; 
        while (count1 >= 10)// This while loop is only needed when the number of people in the room,count1 is more than 10
        {
            ...
            count1 = count1 - 10.0;/*This is to prevents an infinite loop the from happeningand so that this while loop runs until count1 is
